...
def get_ds(config):
    ...
    val_ds = BilingualDataset(val_ds_raw, tokenizer_src, tokenizer_tgt, seq_len = config["seq_len"], src_lang = config["lang_src"], tgt_lang = config["lang_tgt"])
...
def train_model(config):
    ...
    for epoch in range(initail_epoch, config["num_epochs"]):
        ...
        run_validation(model, val_loader, tokenizer_src, tokenizer_tgt, config["seq_len"], device, print_msg, global_step, writer, num_examples=2)
