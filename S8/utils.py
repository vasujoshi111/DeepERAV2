...
def get_train_parameters(norm = "bn"):
    ...
    if norm.lower() == "bn":
        model = BNNet().to(device) # Set the model to same device.
    elif norm.lower() == "gn":
        model = GNNet().to(device) # Set the model to same device.
    elif norm.lower() == "ln":
        model = LNNet().to(device) # Set the model to same device.
    ...
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=6, gamma=0.1, verbose=True) # Initialize the scheduler learning rate.
